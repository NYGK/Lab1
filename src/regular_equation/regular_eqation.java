...
public class regular_eqation {
	...
	public void simplify( String ins)
	{
		...
		int num = Integer.parseInt( ins.substring(12 , ins.length() ) );
		...
		while(p != null)
		{
			...
			for( int i = 0 ; i < 26;i++)
			{
				if('a' + i  == variable )
				{
					temp.coe=temp.coe*math.pow(num, p.variable_list[i]);
...
//-----------------   输入命令---------------------------//
		while(true)
		{
			System.out.println("请输入命令：（输入!exist退出）");
		    input =in.nextLine();
